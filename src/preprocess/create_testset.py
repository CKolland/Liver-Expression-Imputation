...
def create_testset(path_to_adata: str, split: float):
    ...
    logging.info(C.TESTSET_READ_MSG.format(adata=adata))
    ...
    logging.info(C.TESTSET_CREATED_SPLIT_MSG.format(size=int(split_size)))
    ...
    logging.info(C.TESTSET_TESTSET_MSG.format(test_data=adata_test))

    # Keep rest of training data
    adata_train = adata[~test_mask, :].copy()
    logging.info(C.TESTSET_TRAINSET_MSG.format(train_data=adata_train))
    ...
    logging.info(C.TESTSET_SAVE_MSG.format(out_dir=adata_path.parent))
